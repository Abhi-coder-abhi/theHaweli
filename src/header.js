...
const Header = () => {
 ...
 const handleLoginSubmit = (e) => {
  ...
  const loginData = {
    email: loginMail, // The email entered in the login form
    password: loginPassword, // The password entered in the login form
  };

  // Make a POST request to the server to check the login
  fetch("http://localhost:5511/checkLogin", {
    method: "POST",
  ...
  const handleSubmit = (e) => {
    ...
    fetch("http://localhost:5511/sendData", {
      method: "POST",
      headers: {
  ...
  return (
    <div>
      <header className="fixed-header" style={headerStyle}>
        ...
        <button style={{ marginRight: "10px", fontSize: '18px', borderRadius: '10px', padding: '10px 20px', cursor: 'pointer' }} onClick={openRegisterModal} disabled={loginStatus}>Register</button>
          <button style={{ marginRight: "30px", fontSize: '18px', borderRadius: '10px', padding: '10px 20px', cursor: 'pointer' }} onClick={openLoginModal} disabled={loginStatus || registerStatus}>Login</button>
